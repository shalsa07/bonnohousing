...
export default function LegalPopups() {
  ...
  return (
    (siteState.disclaimerPopup || siteState.termsConditionsPopup || siteState.privacyPolicyPopup || siteState.copyrightNoticePopup) && <div className='legal-popup flex fixed top-0 left-0 right-0 bottom-0 z-[100] justify-center items-center bg-black/65 bg-opacity-50 w-full h-full overflow-y-auto'>
        <LegalPageWrapperPopup>
            ...
            <button className={`flex fixed bottom-4 left-0 right-0 mx-auto w-lg h-12 mt-10 items-center rounded-full text-gray-100 uppercase justify-center ${settings.bonnoBlue}`} onClick={()=>siteDispatch({type:ACTIONS_SITE.CLOSE_ALL_POPUPS})}>close</button>
        </LegalPageWrapperPopup>
    </div>)
}
